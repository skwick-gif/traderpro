  ...
  return (
    <div className="app">
      ...
      <div className="main-content">
        ...
        <main className={`content-area ${sidebarItems.length > 0 ? 'with-sidebar' : ''}`}>
          {activeTab === 'dashboard' && <Dashboard />}
          {activeTab !== 'dashboard' && (
            <div style={{ textAlign: 'center', marginTop: '4rem', color: '#94a3b8' }}>
              <h2 style={{ fontSize: '1.5rem', fontWeight: 'bold', marginBottom: '1rem' }}>
                {currentTabConfig?.label}
                {activeSubTab && ` - ${currentSubTabConfig?.label}`}
              </h2>
              <p>Content for this section is not yet implemented</p>
            </div>
          )}
        </main>
      </div>
